...
public class ReportDAO {
    ...
    // NEW: Employer sees only EMPLOYEES in their own department
    public List<Map<String, Object>> getUsersReportByDepartment(String department) {
        List<Map<String, Object>> users = new ArrayList<>();
        String sql = "SELECT user_id, username, full_name, email, role, department, " +
    ...
    public Map<String, Object> getUserDetails(int userId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (rs1.next()) {
                ...
                details.put("isActive", rs1.getBoolean("is_active"));
    ...
    // NEW: Check if a user belongs to a given department (for employer access control)
    public boolean isUserInDepartment(int userId, String department) {
        String sql = "SELECT COUNT(*) FROM users WHERE user_id = ? AND department = ? AND role = 'EMPLOYEE'";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    // NEW: Bad word stats scoped to a department (employer view)
    public Map<String, Integer> getBadWordStatsByDepartment(String department) {
        Map<String, Integer> stats = new HashMap<>();
        try (Connection conn = DatabaseConnection.getConnection()) {
            String sql1 = "SELECT COUNT(*) as count FROM messages m " +
            ...
            String sql2 = "SELECT COUNT(DISTINCT m.sender_id) as count FROM messages m " +
                         "JOIN users u ON m.sender_id = u.user_id " +
                         "WHERE m.is_censored = TRUE AND u.department = ? AND u.role = 'EMPLOYEE'";
            PreparedStatement stmt2 = conn.prepareStatement(sql2);
    ...
    String sql = "SELECT u.user_id, u.full_name, u.department, u.role, " +
                "COUNT(m.message_id) as violation_count " +
                "FROM users u " +
                "JOIN messages m ON m.sender_id = u.user_id " +
                "WHERE m.is_censored = TRUE " +
                // removed any role filter so both EMPLOYEE and EMPLOYER appear
                "GROUP BY u.user_id, u.full_name, u.department, u.role " +
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            row.put("role", rs.getString("role"));  // ✅ include role
            row.put("violationCount", rs.getInt("violation_count"));
            list.add(row);
        }
    } catch (SQLException e) {
        e.printStackTrace();
    ...
    String sql = "SELECT u.user_id, u.full_name, u.department, " +
                ...
                "JOIN messages m ON m.sender_id = u.user_id " +
                "WHERE m.is_censored = TRUE " +
                "AND u.role = 'EMPLOYEE' " +       // ✅ employers only see employees
                "AND u.department = ? " +           // ✅ only their department
    ...
    public List<Map<String, Object>> getCensoredMessagesByUser(int userId) {
    List<Map<String, Object>> messages = new ArrayList<>();
    String sql = "SELECT m.message_id, m.subject, m.message_content, m.original_content, " +
                "m.sent_at, sender.user_id as sender_id, sender.full_name as sender_name, " +
                "receiver.full_name as receiver_name " +
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            map.put("messageContent", rs.getString("message_content"));   // ✅ correct column
            map.put("originalContent", rs.getString("original_content")); // ✅ correct column
            map.put("sentAt", rs.getTimestamp("sent_at"));
...
public List<Map<String, Object>> getCensoredMessagesByUserAndDepartment(int userId, String department) {
    List<Map<String, Object>> messages = new ArrayList<>();
    String sql = "SELECT * FROM messages WHERE contains_bad_word = 1 AND sender_id = ? AND department = ?";
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            map.put("receiverName", rs.getString("receiver_name"));
            map.put("messageContent", rs.getString("filtered_message"));
            map.put("originalContent", rs.getString("original_message"));
            map.put("subject", rs.getString("subject"));
            map.put("sentAt", rs.getTimestamp("sent_at"));
            messages.add(map);
        }
...
public Map<String, Object> getViolatorByUserId(int userId) {
    ...
    String sql = "SELECT u.user_id, u.full_name, u.department, u.role, COUNT(m.message_id) AS violationCount " +
                 "FROM users u " +
                 "JOIN messages m ON u.user_id = m.sender_id " +
                 "WHERE u.user_id = ? AND m.is_censored = TRUE " +
                 "GROUP BY u.user_id, u.full_name, u.department, u.role";
    try (Connection conn = DatabaseConnection.getConnection();
         PreparedStatement stmt = conn.prepareStatement(sql)) {
        stmt.setInt(1, userId);
        ...
        if (rs.next()) {
            ...
            violator.put("role", rs.getString("role"));
            violator.put("violationCount", rs.getInt("violationCount"));
        }
    } catch (SQLException e) {
        e.printStackTrace();
    }
...
// For Employer (department-specific)
public Map<String, Object> getViolatorByUserIdAndDept(int userId, String department) {
    Map<String, Object> violator = null;
    String sql = "SELECT u.user_id, u.full_name, u.department, u.role, COUNT(m.message_id) AS violationCount " +
                 "FROM users u " +
                 "JOIN messages m ON u.user_id = m.sender_id " +
                 "WHERE u.user_id = ? AND u.department = ? AND m.is_censored = TRUE " +
                 "GROUP BY u.user_id, u.full_name, u.department, u.role";
    try (Connection conn = DatabaseConnection.getConnection();
         PreparedStatement stmt = conn.prepareStatement(sql)) {
        stmt.setInt(1, userId);
    ...
    public List<Map<String, Object>> getMessageActivity() {
        ...
        String sql = "SELECT DATE(sent_at) as date, COUNT(*) as count " +
                    "FROM messages GROUP BY DATE(sent_at) ORDER BY date DESC LIMIT 30";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
    ...
    // NEW: Message activity for employer's department only
    public List<Map<String, Object>> getMessageActivityByDepartment(String department) {
        List<Map<String, Object>> activity = new ArrayList<>();
        String sql = "SELECT DATE(m.sent_at) as date, COUNT(*) as count " +
                    "FROM messages m " +
                    "JOIN users u ON m.sender_id = u.user_id " +
                    "WHERE u.department = ? AND u.role = 'EMPLOYEE' " +
                    "GROUP BY DATE(m.sent_at) ORDER BY date DESC LIMIT 30";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, department);
            ResultSet rs = stmt.executeQuery();
    ...
    public List<Map<String, Object>> getTopMessageSenders() {
        ...
        String sql = "SELECT u.user_id, u.full_name, u.department, u.role, COUNT(m.message_id) as message_count " +
                    "FROM messages m " +
                    "JOIN users u ON m.sender_id = u.user_id " +
                    "GROUP BY u.user_id ORDER BY message_count DESC LIMIT 10";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                ...
                sender.put("role", rs.getString("role"));
    ...
    // NEW: Top message senders in employer's department (employees only)
    public List<Map<String, Object>> getTopMessageSendersByDepartment(String department) {
        List<Map<String, Object>> senders = new ArrayList<>();
    ...
    public List<Map<String, Object>> getEmployeePerformance() {
        ...
        String sql = "SELECT u.user_id, u.full_name, u.department, " +
                    ...
                    "GROUP BY u.user_id ORDER BY completed_tasks DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                performance.add(buildPerformanceMap(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // NEW: Performance scoped to employer's department only
    public List<Map<String, Object>> getEmployeePerformanceByDepartment(String department) {
        List<Map<String, Object>> performance = new ArrayList<>();
        String sql = "SELECT u.user_id, u.full_name, u.department, " +
                    "COUNT(t.task_id) as total_tasks, " +
                    "SUM(CASE WHEN t.status = 'COMPLETED' THEN 1 ELSE 0 END) as completed_tasks " +
                    "FROM users u " +
                    "LEFT JOIN tasks t ON u.user_id = t.assigned_to " +
                    "WHERE u.role = 'EMPLOYEE' AND u.is_active = TRUE AND u.department = ? " +
                    "GROUP BY u.user_id ORDER BY completed_tasks DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, department);
    ...
    private Map<String, Object> buildPerformanceMap(ResultSet rs) throws SQLException {
    ...
    public List<Map<String, Object>> getLeavesByStatus(String status) {
        ...
        String sql = "SELECT l.leave_id, l.leave_type, l.start_date, l.end_date, l.total_days, " +
                    ...
                    "WHERE l.status = ? ORDER BY l.applied_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, status);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                leaves.add(buildLeaveMap(rs));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // NEW: Leaves by status scoped to employer's department
    public List<Map<String, Object>> getLeavesByStatusAndDepartment(String status, String department) {
        List<Map<String, Object>> leaves = new ArrayList<>();
        String sql = "SELECT l.leave_id, l.leave_type, l.start_date, l.end_date, l.total_days, " +
                    "l.reason, l.status, l.applied_at, emp.full_name as employee_name, " +
                    "emp.department, mgr.full_name as manager_name " +
                    ...
                    "JOIN users emp ON l.employee_id = emp.user_id " +
                    "LEFT JOIN users mgr ON l.approved_by = mgr.user_id " +
                    "WHERE l.status = ? AND emp.department = ? AND emp.role = 'EMPLOYEE' " +
    ...
    private Map<String, Object> buildLeaveMap(ResultSet rs) throws SQLException {
    ...
    // NEW: Department-scoped stats for employer
    public Map<String, Integer> getOverallStatsByDepartment(String department, int employerId) {
        Map<String, Integer> stats = new HashMap<>();
        try (Connection conn = DatabaseConnection.getConnection()) {
            stats.put("totalEmployees", getStatParam(conn,
                "SELECT COUNT(*) FROM users WHERE role = 'EMPLOYEE' AND is_active = TRUE AND department = ?", department));
            stats.put("totalMessages", getStatParam(conn,
                "SELECT COUNT(*) FROM messages m JOIN users u ON m.sender_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE'", department));
            stats.put("totalTasks", getStatInt(conn,
                "SELECT COUNT(*) FROM tasks WHERE assigned_by = ?", employerId));
            stats.put("completedTasks", getStatInt(conn,
                "SELECT COUNT(*) FROM tasks WHERE assigned_by = ? AND status = 'COMPLETED'", employerId));
            stats.put("pendingTasks", getStatInt(conn,
                "SELECT COUNT(*) FROM tasks WHERE assigned_by = ? AND status = 'PENDING'", employerId));
            stats.put("totalLeaves", getStatParam(conn,
                "SELECT COUNT(*) FROM leaves l JOIN users u ON l.employee_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE'", department));
            stats.put("approvedLeaves", getStatParam2(conn,
                "SELECT COUNT(*) FROM leaves l JOIN users u ON l.employee_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE' AND l.status = 'APPROVED'", department));
            stats.put("pendingLeaves", getStatParam2(conn,
                "SELECT COUNT(*) FROM leaves l JOIN users u ON l.employee_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE' AND l.status = 'PENDING'", department));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return stats;
    ...
    private int getStatParam(Connection conn, String sql, String param) throws SQLException {
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private int getStatParam2(Connection conn, String sql, String param) throws SQLException {
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, param);
            ResultSet rs = stmt.executeQuery();
            return rs.next() ? rs.getInt(1) : 0;
    ...
    private int getStatInt(Connection conn, String sql, int param) throws SQLException {
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, param);
            ResultSet rs = stmt.executeQuery();
            return rs.next() ? rs.getInt(1) : 0;
    ...
    // NEW: Leave stats by status for employer's department
    public Map<String, Integer> getLeaveStatsByStatusAndDepartment(String department) {
        Map<String, Integer> stats = new HashMap<>();
        String sql = "SELECT l.status, COUNT(*) as count FROM leaves l " +
                    "JOIN users u ON l.employee_id = u.user_id " +
                    "WHERE u.department = ? AND u.role = 'EMPLOYEE' GROUP BY l.status";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, department);
            ResultSet rs = stmt.executeQuery();
    ...
    public Map<String, Integer> getDepartmentStats() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                if (dept != null && !dept.isEmpty()) stats.put(dept, rs.getInt("count"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    // NEW: Message stats scoped to employer's department
    public Map<String, Integer> getMessageStatsByDepartment(String department) {
        Map<String, Integer> stats = new HashMap<>();
        try (Connection conn = DatabaseConnection.getConnection()) {
            stats.put("total", getStatParam(conn,
                "SELECT COUNT(*) FROM messages m JOIN users u ON m.sender_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE'", department));
            stats.put("read", getStatParam(conn,
                "SELECT COUNT(*) FROM messages m JOIN users u ON m.sender_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE' AND m.is_read = TRUE", department));
            stats.put("unread", getStatParam(conn,
                "SELECT COUNT(*) FROM messages m JOIN users u ON m.sender_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE' AND m.is_read = FALSE", department));
            stats.put("censored", getStatParam(conn,
                "SELECT COUNT(*) FROM messages m JOIN users u ON m.sender_id = u.user_id WHERE u.department = ? AND u.role = 'EMPLOYEE' AND m.is_censored = TRUE", department));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return stats;
    ...
    public List<Map<String, Object>> getMessagesBySenderRole(String role) {
        List<Map<String, Object>> messages = new ArrayList<>();
        String sql = "SELECT m.message_id, m.subject, m.message_content, m.attachment_path, m.sent_at, " +
             "sender.user_id as sender_id, sender.full_name as sender_name, " +
             "receiver.full_name as receiver_name " +
             "FROM messages m " +
             "JOIN users sender ON m.sender_id = sender.user_id " +
             "JOIN users receiver ON m.receiver_id = receiver.user_id " +
             "WHERE sender.role = ? ORDER BY m.sent_at DESC";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                msg.put("subject", rs.getString("subject"));
                msg.put("messageContent", rs.getString("message_content"));
                msg.put("attachmentPath", rs.getString("attachment_path"));
                msg.put("sentAt", rs.getTimestamp("sent_at"));
                msg.put("senderId", rs.getInt("sender_id"));
                msg.put("senderName", rs.getString("sender_name"));
                msg.put("receiverName", rs.getString("receiver_name"));
    ...
    public List<Map<String, Object>> getMessagesBySenderRoleByDepartment(String role, String department) {
        ...
        String sql = "SELECT m.message_id, m.subject, m.message_content, m.attachment_path, m.sent_at, " +
             ...
             "JOIN users sender ON m.sender_id = sender.user_id " +
             "JOIN users receiver ON m.receiver_id = receiver.user_id " +
             "WHERE sender.role = ? AND sender.department = ? ORDER BY m.sent_at DESC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, role);
            stmt.setString(2, department);
